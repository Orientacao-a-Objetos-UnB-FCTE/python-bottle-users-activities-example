...
class UserController(BaseController):
    ...
    def add_user(self):
        if request.method == 'GET':
            ...
            return self.render('user_form', user=None, activities=activities, action="/users/add")
        else:
            # POST - salvar usuário
    ...
    def edit_user(self, user_id):
        ...
        if request.method == 'GET':
            activities = self.activity_service.get_all()
            return self.render('user_form', user=user, activities=activities, action=f"/users/edit/{user_id}")
        else:
            # POST - salvar edição
